...
!! called by:
!!
      subroutine writeoptfit_atomic(optepoch,&
...
!! write maximum errors
      if(fitting_unit.eq.1)then
        write(ounit,'(a,f14.6,a21,i8,a2)')' max Etot error in last epoch (train set): ',&
          maxerror_etot_train*tounit,' eV/atom (structure ',imaxerror_etot_train,')' 
        write(ounit,'(a,f14.6,a21,i8,a2)')' max Etot error in last epoch (test set) : ',&
          maxerror_etot_test*tounit, ' eV/atom (structure ',imaxerror_etot_test,')' 
      elseif(fitting_unit.eq.2)then
        write(ounit,'(a,f14.6,a21,i8,a2)')' max Etot error in last epoch (train set): ',&
          maxerror_etot_train*tounit,' Ha/atom (structure ',imaxerror_etot_train,')' 
        write(ounit,'(a,f14.6,a21,i8,a2)')' max Etot error in last epoch (test set) : ',&
          maxerror_etot_test*tounit, ' Ha/atom (structure ',imaxerror_etot_test,')' 
      endif
!!
      return
      end
