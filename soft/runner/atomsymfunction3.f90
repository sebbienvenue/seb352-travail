...
!!
      subroutine atomsymfunction3(i1,i2,iindex,natoms,atomindex,natomsdim,nelem,&
...
!!
              ...
              rjk=dsqrt(rjk)
              if(rjk.le.rmin) then
                write(ounit,*)'atomsymfunction3 atoms too close: rjk=',rjk
                lrmin=.false.
              endif
!!
...
!!
                 temp5=1.d0+f/g
                 if(temp5.lt.0.0d0)temp5=0.0d0
                 temp4=temp4*zeta_local(i2,iindex)*temp5**(zeta_local(i2,iindex)-1)/g**2
...
!! calculation of exponential term
                expxyz=dexp(-eta_local(i2,iindex)*(rij**2+rik**2+rjk**2))
