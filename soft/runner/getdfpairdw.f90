...
!!
!! called by:
!! - optimize_pair_combined.f90
!!

      subroutine getdfpairdw(iatom,ixyz,&
...
!! initializations
      ...
      alphagaussian = 0.5d0
!!
!! the loop over all pairs MUST be here!!! We have to sum for each force component again over all pairs
!! because the dedgdw are different for each pair
...
!! calculate nodes_values = y_i^j for atom i1
        nodes_values(:,:)=0.0d0
        nodes_sum(:,:)   =0.0d0
        itemp=pairindex(zelemp(1,i1),zelemp(2,i1)) 
!!
...
!! calculation of dnodes_values_dg is correct
        ...
        call get_dnodes_values_dg(&
          maxnum_layers_short_pair,num_layers_short_pair(itemp),maxnodes_short_pair,&
          ...
          if(i2.eq.1)then ! 1st hidden layer
            do i3=1,nodes_short_pair(i2,itemp) ! over all nodes in 1st target hidden layer
              do i4=1,num_funcvalues_short_pair(itemp) ! over all input nodes
                ...
                dedgdw(icount,i4,itemp,1)=symfunctionp(i4,i1)*dedgdw(kcount,i4,itemp,1)+dedw(kcount,1) !! BUG DETECTED BY JOVAN
              enddo ! i4
            enddo ! i3
...
!! calculate dfpairdw for atom iatom and component ixyz
        do i2=1,num_funcvalues_short_pair(itemp)
          do i3=1,num_weightspairfree(pairindex(zelemp(1,i1),zelemp(2,i1))) 
            dfpairdw(i3,itemp)=dfpairdw(i3,itemp)  &
            + dsfuncdxyz_pair(i2,i1,iatom,ixyz)*dedgdw(wconstraintpidx(i3,pairindex(zelemp(1,i1),zelemp(2,i1))),i2,itemp,1)
          enddo ! i3
        enddo ! i2
!!
