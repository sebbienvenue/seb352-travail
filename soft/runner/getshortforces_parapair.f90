...
!! - predictionpair.f90
!!
        subroutine getshortforces_parapair(pindex,&
...
!! initialization
      ...
      alphagaussian     = 0.5d0
!!
!! get depairdsfunc for each pair 
!!--------------------------------
      do i1=1,num_pairs 
...
!!
       call calconenn(1,maxnum_funcvalues_short_pair,maxnodes_short_pair,&
           ...
           symfunction_pair,weightsp,nodes_values,nodes_sum,nnoutput,&
...
!! we have to sum over the nodes in the previous layer (i4)
                do i4=1,nodes_short_pair(i2-1,pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1)))) ! sum over all nodes in previous layer
                  ...
                  icount=windex_short_pair(2*i2-1, pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1)))) +&
                     (i4-1)*nodes_short_pair(i2,pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1))))+i3-1 ! set pointer in weight array
...
!! summation over i2 and i4
      ...
      do i1=1,num_atoms  ! over all atoms in structure
        do i3=1,3 ! x,y,z
           do i2=1,num_pairs  ! over all pair in structure
             do i4=1,num_funcvalues_short_pair(pairindex(zelemp(1,pindex(i2)),zelemp(2,pindex(i2))))  ! over all symmetry functions
