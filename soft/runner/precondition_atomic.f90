...
!! CAUTION: THIS SUBROUTINE IS NOT YET READY, CHARGES ARE NOT PROPERLY INCLUDED IN CASE OF SECOND OUTPUT NODE

!! called by:
!!
      subroutine precondition_atomic(ntrain,trainelem,&
...
!!
      ...
      if(lelec)then
        write(*,*)'ERROR: precondition_short_atomic is not yet prepared for electrostatics from second NN output'
        stop !'
      endif
!!
...
!! scale the short range connecting weights between final hidden layer and output layer
          ...
          do i1=1,nelem !'
            do i2=windex_short_atomic(2*num_layers_short_atomic(i1)-1,i1),&
              windex_short_atomic(2*num_layers_short_atomic(i1),i1)
...
!! shift the short range bias weights
          do i1=1,nelem
            weights_short_atomic(num_weights_short_atomic(i1),i1)&
              =weights_short_atomic(num_weights_short_atomic(i1),i1)&
               -(prefactor*avnneshort-eshortav)   
          enddo
        endif ! lshort
!!
        if((mpirank.eq.0).and.lshort.and.lelec)then
          write(ounit,*)'-------------------------------------------------------------'
        endif !'
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
      endif ! mpirank
