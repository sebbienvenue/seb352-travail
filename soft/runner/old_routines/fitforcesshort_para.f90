...
!! - fitting.f90
!!
      subroutine fitforcesshort_para(ounit,nblock,npoints,idx,&
...
!! calculate the error for the current force component
              abserrorf=abs(xyzforce(i2,i3)-nnxyzforce(i2,i3))
              errorf=xyzforce(i2,i3)-nnxyzforce(i2,i3)
              errorf=-1.d0*errorf          ! this must be used!
!!
!!
              if(abserrorf.gt.kalmanthresholdf*rmse_force_s_ref)then
...
!!
                      errorf=errorfsum(i4)/dble(num_atoms_element(i4)) ! CHECK!
!!                      errorf=errorfsum(i4) ! CHECK!
!! if we average over several force derivatives we should normalize here 
                      errorf=errorf/dble(nforce) ! CHECK!
!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!! update the weights:
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!
                        if(mpisize.eq.1)then
                          call updatekalman(ounit,kaldim,corrdim,&
                            ...
                            kalgainmat_list(1,i4),corrmatrix_list(1,i4),&
                            errorf,ldebug)
                        else
...
!!
                        call updatesteepest(ounit,&
                          num_weightsshort,weights,dfshortdw,errorf,ldebug)
!!
