...
!! - fitting.f90
!!
      subroutine writekalmanunformatted(ndim,&
        ...
        if(lshort)then
          do i4=1,ndim
            if(mpirank.eq.0)then
              open(kalunit,file=kalfilenames(i4),form='unformatted',status='replace') 
...
!! collect the correlation matrices of all processes
              if(mpirank.eq.0)then
                allocate(corrmatrixtemp_triangle(num_weightsshortfree(i4)*(num_weightsshortfree(i4)+1)/2))
              endif ! mpirank.eq.0
