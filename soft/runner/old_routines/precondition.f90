...
!! - fitting.f90 
!!
      subroutine precondition(ntrain,trainelem,&
...
!              write(*,*)'charges ',elementindex(zelem_list(i1,i2)),nnatomcharge_list(i1,i2),&
!                  avnncharge(elementindex(zelem_list(i1,i2))),&
!                 (nnatomcharge_list(i1,i2)-avnncharge(elementindex(zelem_list(i1,i2))))**2.d0
            enddo
          enddo
...
!! calculate final standard deviation of short range energies
      if(mpirank.eq.0)then
        ...
        if(lshort)then
          ...
          write(ounit,'(a,f14.6)')' Minimum NN Eshort    ',nneshortmin
          write(ounit,'(a,f14.6)')' Maximum NN Eshort    ',nneshortmax
          write(ounit,'(a,f14.6)')' Average NN Eshort    ',avnneshort
          write(ounit,'(a,f14.6)')' Stddev NN Eshort     ',nnstddev
