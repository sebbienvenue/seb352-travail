...
!!
      ...
      integer i1,i2,i3,i4,i5,i6                        ! internal
...
!! MG: global scaling factor for element-decoupled Kalman filter
      real*8 global_scaling                                                 ! internal
!! MG: local scaling matrix for element-decoupled forces variant 2
      real*8 local_scaling(3,3)                                             ! internal
!! MG: dF/dw holding x, y and z components (required for local_scaling)
      real*8, dimension(:,:,:), allocatable :: dfshortdw_xyz                ! internal 
!!      real*8 dfshortdw_xyz(maxnum_weights_short_atomic,3,nelem)                 ! internal
!! MG: Error vector of force on current atom (required for force update V2)
      real*8 errorf_xyz(3)                                                  ! internal
!!
...
!! determine mforcegroup for each element and make sure it is not larger than the number of forces for this element in this block of structures
      if(ldynforcegroup)then
        nforcegroup=dynforcegroup_step*(countepoch-1)+dynforcegroup_start
        if(nforcegroup.lt.1)then
          nforcegroup=1
        endif
      endif
...
!          write(ounit,*)'JBdebug optimize_short_combined i1 ',i1,num_atoms_list(idx(i1))
!!
...
!!
!! skip this energy if structure is ionic and only forces should be used for fitting 
          if((lionforcesonly).and.(abs(totalcharge_list(idx(i1))).gt.1.0d-10))then
            goto 98 ! 
          endif
!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
!!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!! MG: This does not seem to make sense, goto should lead to other place than 77
...
!! MG: alternative block: if energy of last point is not used, also skip force
!! MG: updates
            if((abserrore.le.errorthreshold).and.(.not.luseedkalman))then
              goto 77 ! don't use this gradient because point is good enough
            elseif((abserrore.le.errorthreshold).and.(luseedkalman))then
              goto 101
            endif
!!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!!
...
!!
!! MG: If element-decoupled Kalman filter requested, compute global scaling factor
                if(luseedkalman)then
                  global_scaling=0d0
!! MG: Fix: deshortdw is usually normalized by the number of weights,
!! MG: Fix: the ED-GEKF needs the original derivatives.
!! MG: Fix2: Moved rescaling to subroutine, now number of atoms is passed.
                  call getglobalscaling(num_weights_short_atomic_free,&
                    deshortdw,corrmatrix_list,maxcorrdim,corrdim,&
                    num_atoms_element,num_atoms,global_scaling)
!! MG: debug        WRITE(*,*) global_scaling
                endif ! luseedkalman
!!
...
!! MG: Check, whether the element-decoupled filter should be used
                        if(luseedkalman)then 
!! MG: Perform element-decoupled update (modified copy of original update subroutine
!! MG: Fix: deshortdw and errore are both normalized by the number of weights,
!! MG: Fix: but the ED-GEKF requires the absolute values.
!! MG: Fix2: Moved rescaling to subroutine, now number of atoms is passed.
!! 
!! MG: FOR FORCES STORE OLD MATRIX
                          if(luseforces)then
                            corrmatrixf_list(:,i2) = corrmatrix_list(:,i2)
                          endif
                          call updatekalman_ed(kaldim(i2),corrdim(i2),&
                            num_weights_short_atomic_free(i2),kalmanlambda(i2),kalmannue,&
                            weights,deshortdw_temp,&
                            corrmatrix_list(1,i2),errore,global_scaling,num_atoms)
                        else ! MG: standard update
...
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!! MG: also skip forces for ED GEKF if energy was not used
            if(ljointefupdate.or.(luseforces.and.luseedkalman))goto 101
!! D            if(ljointefupdate.or.(luseforces.and.luseedkalman))then
!! D                WRITE(*,*) "SKIPPED EVERYTHING!", i1
!! D                goto 101
!! D            endif
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!!
...
!          write(ounit,*)'invneighborlist'
!          do i2=1,natoms
!            do i3=1,max_num_atoms
!              write(ounit,'(3i4,i8)')mpirank,i2,i3,invneighboridx_short_atomic(i2,i3)
!            enddo 
!          enddo
!          stop
!!
...
!! TODO: This should be better done with mpi_gatherv
          ...
          enddo
!! FIXME: This allreduce causes trouble in parallel runs
          call mpi_allreduce(mpi_in_place,dsfuncdxyz,&
            maxnum_funcvalues_short_atomic*max_num_atoms*(max_num_neighbors_short_atomic+1)*3,&
...
!!
!! MG: if 2nd variant of ED force fitting is used, allocate auxiliary array for
!! MG: dF/dw with xyz components here
          if(ledforcesv2)then
            allocate(dfshortdw_xyz(maxnum_weights_short_atomic,3,nelem))
          endif
!!
!! calculate the error of the forces individually for each atom and component
          do i2=1,num_atoms
!!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!! MG: Initialize containers for v2 here. 
            if(ledforcesv2)then
              errorf_xyz(:)=0.0d0
              local_scaling(:,:)=0.0d0
              dfshortdw_xyz(:,:,:)=0.0d0
...
!!
!              write(ounit,*)'JBdebug optimize_short_combined i2,i3 ',i2,i3
!!
...
!! This scales nearly perfectly! 
              ...
              call getoneshortforce_para(i2,i3,max_num_neighbors_short_atomic,&
                ...
                invneighboridx_short_atomic,natoms,atomindex,num_atoms,zelem,&
...
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!! MG: If variant 2 of ED force fitting is used, store
!! MG: errorvector
              if(ledforcesv2)then
                errorf_xyz(i3)=errorf
              endif
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!! MG: In case ED-GEKF energy update happened, all forces should be used
                !! .or.&
                !! luseedkalman&
!! MG: v2 update should not be skipped at this point
                .or.&
                ledforcesv2&
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                .or.&
                ...
                .or.&
                (lfgroupbystruct.and.(count_atoms_element_f(elementindex(zelem(i2)))&
                  .eq.num_atoms_element_struct(elementindex(zelem(i2))))&
                  .and.(i3.eq.3)))then                                  ! last force of this element in this structure, update may be necessary 
!!
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!! MG: if V2 is used, i3 is at the z-component and 2-norm of errorvector 
!! MG: errorf_xyz is smaller than errorthresholdf, skip the update:
                if(ledforcesv2.and.(i3.eq.3).and.&
                  ((dsqrt(errorf_xyz(1)**2+errorf_xyz(2)**2+errorf_xyz(3)**2)).le.errorthresholdf))then
!!                  WRITE(*,*) "SKIP"
                  goto 88
                endif
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!! MG: In case ED-GEKF energy update happened, all forces should be used
                !! if((abserrorf.le.errorthresholdf).and.(.not.(ljointefupdate.or.luseedkalman))) goto 88
                if((abserrorf.le.errorthresholdf).and.(.not.(ljointefupdate.or.ledforcesv2))) goto 88
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!!
...
!! serial version
                ...
                if(paramode.eq.1)then
                  if(mpirank.eq.0)then
                    call getdfshortdw(i2,i3,natoms,&
...
!! WARNING: this version still crashes after a while
                  call getdfshortdw_para(i2,i3,max_num_neighbors_short_atomic,&
...
!! MG: This should NOT be done for the second version of the force fitting.
!! MG: Since the associated algorithm tries to fit the force vector, it is
!! MG: important to keep the direction of the force gradient unchanged.
                if(.not.ledforcesv2)then
...
!!                      write(ounit,*)'setting lenforceupdatef'
                ...
                enddo ! i4 
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!! MG: Compute scaling factor for force update if standard force fitting is used
!! MG: Variant 2 uses a scaling matrix local_scaling
!! MG: Change num_atoms_element to num_atoms_element_struct, as the former gets
!! MG: reset
                if(luseedkalman.and.(.not.ledforcesv2))then
                  call getforcescaling(num_weights_short_atomic_free,&
                    dfshortdw,corrmatrixf_list,maxcorrdim,corrdim,&
                    num_atoms_element_struct,num_atoms,global_scaling,&
                    scalefactorftemp(elementindex(zelem(i2))))
!!                  WRITE(*,*) global_scaling, "GS"
                endif
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!!
...
!! MG: This expanded if-clause is required by ED-GEKF for forces if some
!! MG: if some elements are not present in all structures, else the update is
!! MG: stopped too early
                    .and.((count_atoms_element_f(i4).gt.0).or.ledforcesv2))then   
!!
...
!!
!! allocate temporary arrays for free weights
!! MG: Only do this if 2nd force fitting variant is not used
                    if(.not.ledforcesv2)then
...
!!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!! MG: This should not be done for element decoupled filter!
!! MG: In the element decoupled filter, we have our scaling matrices, which
!! MG: allow us to perform updates for all elements, this means we do not
!! MG: differentiate between errorf for elements, but use the one errorf computed for
!! MG: the current component. Because of this the grouping used here cannot be
!! MG: applied.
                    if(.not.luseedkalman)then
...
!!
!! MG: If standard force fitting is used, set corrmats equal
                      if(.not.(lsepkalman.or.luseedkalman))then
...
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                          if(luseedkalman)then
!! MG: Use second version of force fitting if requested
                            if(ledforcesv2)then 
!! 
!! MG: Store xyz components of dF/dw for v2 force fitting
                              dfshortdw_xyz(:,i3,i4)=dfshortdw(:,i4)
!! MG: IF FINISHED, DO UPDATE
                              if((i3.eq.3).and.(i4.eq.nelem))then
                     ! 1) compute local_scaling matrix (change num_atoms_element
                     !    to num_atoms_element_struct, since the former gets
                     !    reset
                                call getlocalscaling(num_weights_short_atomic_free,&
                                  dfshortdw_xyz,corrmatrixf_list,maxcorrdim,corrdim,&
                                  num_atoms_element_struct,num_atoms,local_scaling,&
                                  scalefactorftemp(elementindex(zelem(i2))))
                     ! 2) Update weights
                                do i5=1,nelem
!! MG: Skip update if element not present
                                  if(num_atoms_element_struct(i5).eq.0)then
                                    continue
                                  else  
                                  ! MG: a) allocate temporary arrays
                                    allocate(weights(num_weights_short_atomic_free(i5)))
                                    allocate(dfshortdw_xyz_temp(num_weights_short_atomic_free(i5),3))
                                  ! MG: b) reduce arrays to free weights
                                    do i6=1,num_weights_short_atomic_free(i5)
                                      weights(i6)=weights_short_atomic(wconstraintidx(i6,i5),i5)
                                      dfshortdw_xyz_temp(i6,1)=dfshortdw_xyz(i6,1,i5)
                                      dfshortdw_xyz_temp(i6,2)=dfshortdw_xyz(i6,2,i5)
                                      dfshortdw_xyz_temp(i6,3)=dfshortdw_xyz(i6,3,i5)
                                    enddo ! i6
                                  ! MG: c) perform update (finally...)
                                    call updateforces_edv2(corrdim(i5),&
                                      num_weights_short_atomic_free(i5),num_atoms,&
                                    local_scaling,weights,dfshortdw_xyz_temp,&
                                    corrmatrixf_list(1,i5),errorf_xyz)
                                  ! MG: d) expand weights
                                    do i6=1,num_weights_short_atomic_free(i5)
                                      weights_short_atomic(wconstraintidx(i6,i5),i5)=weights(i6)
                                    enddo ! i6
                                  ! MG: e) deallocate temporary arrays
                                    deallocate(weights)
                                    deallocate(dfshortdw_xyz_temp)
                                  endif ! num_atoms_element_struct(i5).eq.0
                                enddo ! i5
                              endif ! i3=z, i4=nelem
!! MG: otherwise do normal ED force update
                            else
!!D                              WRITE(*,*) i2, i3, i4, "EDA", errorf
                              call updateforces_ed(corrdim(i4),&
                                num_weights_short_atomic_free(i4),num_atoms,&
                                global_scaling,weights,dfshortdw_temp,&
                                corrmatrixf_list(1,i4),errorf,&
                                scalefactorftemp(elementindex(zelem(i2))))
!!D                              WRITE(*,*) i2, i3, i4, "EDB", errorf
!! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                            endif ! ledforcesv2
                          else
...
!! caution: we need to avoid double counting for numbere here (in case we count seperately the update for
!! each element, what we don't want to)
!! => we count this update only for first element i4
                        if(i4.eq.1) numbere=numbere+1
 200                    continue
                      endif ! lrepeate
...
!! MG: Only do this if 2nd force fitting variant is not used, else updated
!! MG: weights will be overwritten with old ones.
                    if(.not.ledforcesv2)then
                      do i5=1,num_weights_short_atomic_free(i4)
                        weights_short_atomic(wconstraintidx(i5,i4),i4)=weights(i5)
                      enddo ! i5
...
!!
!! MG: Only deallocate if 2nd force fitting variant is not used
                    if(.not.ledforcesv2)then
                      deallocate(weights)
...
!!!!!!!!!!!!!!!!!!!!!!!!!
!! MG: It does not make sense to set errorf to 0 here, it should be moved out of
!! the i4(nelem) loop:
!!                    errorf                   = 0.0d0
!!!!!!!!!!!!!!!!!!!!!!!!!
                    errorfsum(i4)            = 0.0d0
...
!! jump here if force should be skipped for weight update
 99           continue
!!!!!!!!!!!!!!!!!!!!!!!!!
!! MG: Moved out errorf reinitialisation of the i4 loop. The ED-GEKF updates use
!! MG: the same errorf for every elemental subupdate, thus it should not be reset in
!! MG: between.
              errorf = 0.0d0
!!!!!!!!!!!!!!!!!!!!!!!!!
!!
...
!!
!! MG: If 2nd variant of ED force fitting is used, deallocate auxiliary
!! MG: dF(xyz)/dw here
          if(ledforcesv2)then
            deallocate(dfshortdw_xyz)
          endif
!!
