...
!!
      ...
      integer nodes_s_temp(0:maxnum_layers_s)                    ! out
      integer nodes_hexton_temp(0:maxnum_layers_hexton)                    ! out
      integer nodes_hextoff_temp(0:maxnum_layers_hextoff)                    ! out
      integer nodes_dens_temp(0:maxnum_layers_dens)                    ! out 
...
!!
      ...
      character*2 elementdummy(3)                       ! internal
!!
      logical lshort_local                              ! internal
      logical lelec_local                               ! internal
      logical lnntb_local                               ! internal
      logical lham_local
      logical loverlap_local                               ! internal
      logical lhexton_local                               ! internal
      logical lhextoff_local                               ! internal
      logical ldens_local                               ! internal
!!
!! local initializations
      lshort_local=.false.
      lelec_local =.false.
      lnntb_local =.false.
      loverlap_local=.false.
      lhexton_local=.false.
      lhextoff_local=.false.
      ldens_local=.false.
...
!!
      elseif(keyword.eq.'check_input_forces')then
        if(count_lcheckinputforces.gt.0)then
          write(ounit,*)'Error: check_input_forces specified twice'
          stop
        endif
        count_lcheckinputforces=count_lcheckinputforces+1
        backspace(nnunit)
        lcheckinputforces=.true.
        read(nnunit,*,ERR=99)dummy,inputforcethreshold
!!
      elseif(keyword.eq.'use_ipi')then
        if(count_useipi.gt.0)then
          write(ounit,*)'Error: use_ipi specified twice'
          stop
        endif
        count_useipi=count_useipi+1
        backspace(nnunit)
        luseipi=.true.
        read(nnunit,*,ERR=99)dummy,desc_str,ipistring,ipisocket
!!
      elseif(keyword.eq.'ion_forces_only')then
        if(count_lionforcesonly.gt.0)then
          write(ounit,*)'Error: ion_forces_only specified twice'
          stop
        endif
        count_lionforcesonly=count_lionforcesonly+1
        lionforcesonly=.true.
!!
...
!!          stop
!!        endif        
!!        count_nn_type_nntb=conut_nn_type_nntb+1
        !!backspace(nnunit)
!!
!!      CMH the above check is redundant as we sort out the specifc nntb mode using some other keywords        
!!
...
!!
      elseif(keyword.eq.'detect_saturation')then
        if(count_ldetect_saturation.gt.0)then
          write(ounit,*)'Error: detect_saturation specified twice'
          stop
        endif
        count_ldetect_saturation=count_ldetect_saturation+1
        ldetect_saturation=.true.
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,saturation_threshold
!!
      elseif(keyword.eq.'data_clustering')then
        if(count_ldataclustering.gt.0)then
          write(ounit,*)'Error: data_clustering specified twice'
          stop
        endif
        count_ldataclustering=count_ldataclustering+1
        ldataclustering=.true.
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,dataclusteringthreshold1,dataclusteringthreshold2
!!
...
!!
      elseif(keyword.eq.'use_ham')then
        if(count_lham.gt.0)then
          write(ounit,*)'Error: use_ham specified twice'
          stop
        endif
        count_lham=count_lham+1
        lham_local=.true.
        if(nntb_flag(0).neqv.lham_local)then
          write(ounit,*)'ERROR in determination of nntb_flag 0'
          stop
        endif
!!
      elseif(keyword.eq.'use_overlap')then
        if(count_loverlap.gt.0)then
          write(ounit,*)'Error: use_overlap specified twice'
          stop
        endif
        count_loverlap=count_loverlap+1
        loverlap_local=.true.
        if(nntb_flag(1).neqv.loverlap_local)then
          write(ounit,*)'ERROR in determination of nntb_flag 1'
          stop
        endif
!!
      elseif(keyword.eq.'use_hexton')then
        if(count_lhexton.gt.0)then
          write(ounit,*)'Error: use_hexton specified twice'
          stop
        endif
        count_lhexton=count_lhexton+1
        lhexton_local=.true.
        if(nntb_flag(2).neqv.lhexton_local)then
          write(ounit,*)'ERROR in determination of nntb_flag 2'
          stop
        endif
!!
      elseif(keyword.eq.'use_hextoff')then
        if(count_lhextoff.gt.0)then
          write(ounit,*)'Error: use_hextoff specified twice'
          stop
        endif
        count_lhextoff=count_lhextoff+1
        lhextoff_local=.true.
        if(nntb_flag(3).neqv.lhextoff_local)then
          write(ounit,*)'ERROR in determination of nntb_flag 3'
          stop
        endif
!!
      elseif(keyword.eq.'use_dens')then
        if(count_ldens.gt.0)then
          write(ounit,*)'Error: use_dens specified twice'
          stop
        endif
        count_ldens=count_ldens+1
        ldens_local=.true.
        if(nntb_flag(4).neqv.ldens_local)then
          write(ounit,*)'ERROR in determination of nntb_flag 4'
          stop
        endif
!!
...
!!      maxnum_layersham is read before in getdimensions
      elseif(keyword.eq.'global_hidden_layers_s')then
        if(count_num_layers_s.gt.0)then
          write(ounit,*)'Error: global_hidden_layers_s specified twice'
          stop !'
        endif
        count_num_layers_s=count_num_layers_s+1
      elseif(keyword.eq.'global_hidden_layers_hexton')then
        if(count_num_layers_hexton.gt.0)then
          write(ounit,*)'Error: global_hidden_layers_hexton specified twice'
          stop !'
        endif
        count_num_layers_hexton=count_num_layers_hexton+1
      elseif(keyword.eq.'global_hidden_layers_hextoff')then
        if(count_num_layers_hextoff.gt.0)then
          write(ounit,*)'Error: global_hidden_layers_hextoff specified twice'
          stop !'
        endif
        count_num_layers_hextoff=count_num_layers_hextoff+1
      elseif(keyword.eq.'global_hidden_layers_dens')then
        if(count_num_layers_dens.gt.0)then
          write(ounit,*)'Error: global_hidden_layers_dens specified twice'
          stop !'
        endif
        count_num_layers_dens=count_num_layers_dens+1
...
!!
      ...
      elseif(keyword.eq.'global_nodes_s')then
        if(lnntb.and.nntb_flag(1))then
          if(count_nodes_s.gt.0)then
            write(ounit,*)'Error: global_nodes_s specified twice'
            stop
          endif
          count_nodes_s=count_nodes_s+1
          backspace(nnunit)
          read(nnunit,*,ERR=99)dummy,(nodes_s_temp(i1),i1=1,maxnum_layers_s-1)
          do i1=1,npairs
            do i2=1,maxnum_layers_s
              nodes_s(i2,i1)=nodes_s_temp(i2)
            enddo
          enddo
        endif
      elseif(keyword.eq.'global_nodes_hexton')then
        if(lnntb.and.nntb_flag(2))then
          if(count_nodes_hexton.gt.0)then
            write(ounit,*)'Error: global_nodes_hexton specified twice'
            stop
          endif
          count_nodes_hexton=count_nodes_hexton+1
          backspace(nnunit)
          read(nnunit,*,ERR=99)dummy,(nodes_hexton_temp(i1),i1=1,maxnum_layers_hexton-1)
          do i1=1,npairs
            do i2=1,maxnum_layers_hexton
              nodes_hexton(i2,i1)=nodes_hexton_temp(i2)
            enddo
          enddo
        endif
      elseif(keyword.eq.'global_nodes_hextoff')then
        if(lnntb.and.nntb_flag(3))then
          if(count_nodes_hextoff.gt.0)then
            write(ounit,*)'Error: global_nodes_hextoff specified twice'
            stop
          endif
          count_nodes_hextoff=count_nodes_hextoff+1
          backspace(nnunit)
          read(nnunit,*,ERR=99)dummy,(nodes_hextoff_temp(i1),i1=1,maxnum_layers_hextoff-1)
          if(mode.eq.2)then
            i1=1
              do i2=1,maxnum_layers_hextoff
                nodes_hextoff(i2,i1)=nodes_hextoff_temp(i2)
              enddo
      ...
      elseif(keyword.eq.'global_nodes_dens')then
        if(lnntb.and.nntb_flag(4))then
          if(count_nodes_dens.gt.0)then
            write(ounit,*)'Error: global_nodes_ham specified twice'
            stop
          endif
          count_nodes_dens=count_nodes_dens+1
          backspace(nnunit)
          read(nnunit,*,ERR=99)dummy,(nodes_dens_temp(i1),i1=1,maxnum_layers_dens-1)
          do i1=1,npairs
            do i2=1,maxnum_layers_dens
              nodes_dens(i2,i1)=nodes_dens_temp(i2)
            enddo
          enddo
        endif
...
!!
      elseif(keyword.eq.'force_threshold')then
        if(count_fitfthres.gt.0)then
          write(ounit,*)'Error: force_threshold specified twice'
          stop
        endif
        count_fitfthres=count_fitfthres+1
        backspace(nnunit)
        lfitfthres=.true.
        read(nnunit,*,ERR=99)dummy,fitfthres
!!
...
!!
      elseif(keyword.eq.'optmode_hextoff')then
        if(count_optmodehextoff.gt.0)then
          write(ounit,*)'Error: optmode_short_energy specified twice'
          stop
        endif
        count_optmodehextoff=count_optmodehextoff+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,optmodehextoff
!!
...
!!
      elseif(keyword.eq.'optmode_hextoff')then
        if(count_optmodehextoff.gt.0)then
          write(ounit,*)'Error: optmode_hextoff specified twice'
          stop
        endif
        count_optmodehextoff=count_optmodehextoff+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,optmodehextoff
!!
      elseif(keyword.eq.'optmode_ham')then
        if(count_optmodeham.gt.0)then
          write(ounit,*)'Error: optmode_ham specified twice'
          stop
        endif
        count_optmodeham=count_optmodeham+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,optmodeham
!!
      elseif(keyword.eq.'optmode_dens')then
        if(count_optmodedens.gt.0)then
          write(ounit,*)'Error: optmode_dens specified twice'
          stop
        endif
        count_optmodedens=count_optmodedens+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,optmodedens
!!
      elseif(keyword.eq.'optmode_hexton')then
        if(count_optmodehexton.gt.0)then
          write(ounit,*)'Error: optmode_hexton specified twice'
          stop
        endif
        count_optmodehexton=count_optmodehexton+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,optmodehexton
!!
      elseif(keyword.eq.'optmode_s')then
        if(count_optmodes.gt.0)then
          write(ounit,*)'Error: optmode_s specified twice'
          stop
        endif
        count_optmodes=count_optmodes+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,optmodes
!!
...
!!
      elseif(keyword.eq.'hextoff_training_triplet')then
        if(count_hextoff_training_triplet.gt.0)then
          write(ounit,*)'Error: hextoff_training_triplet specified twice'
          stop
        endif
        count_hextoff_training_triplet=count_hextoff_training_triplet+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,(elementdummy(i1),i1=1,3)
        call nuccharge(elementdummy(1),hextoff_training_triplet(1))
        call nuccharge(elementdummy(2),hextoff_training_triplet(2))
        call nuccharge(elementdummy(3),hextoff_training_triplet(3))
!! make sure that the heavier element comes first
        if(hextoff_training_triplet(1).lt.hextoff_training_triplet(2))then
          itemp=hextoff_training_triplet(1)
          hextoff_training_triplet(1)=hextoff_training_triplet(2)
          hextoff_training_triplet(2)=itemp
        endif
!!
...
!!
      elseif(keyword.eq.'scale_min_s')then
        if(count_scmin_s.gt.0)then
          write(ounit,*)'Error: scale_min_ham specified twice'
          stop
        endif
        count_scmin_s=count_scmin_s+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,scmin_s
!!
      elseif(keyword.eq.'scale_max_s')then
        if(count_scmax_s.gt.0)then
          write(ounit,*)'Error: scale_max_s specified twice'
          stop
        endif
        count_scmax_s=count_scmax_s+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,scmax_s
!!
      elseif(keyword.eq.'scale_min_hexton')then
        if(count_scmin_hexton.gt.0)then
          write(ounit,*)'Error: scale_min_hexton specified twice'
          stop
        endif
        count_scmin_hexton=count_scmin_hexton+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,scmin_hexton
!!
      elseif(keyword.eq.'scale_max_hexton')then
        if(count_scmax_hexton.gt.0)then
          write(ounit,*)'Error: scale_max_hexton specified twice'
          stop
        endif
        count_scmax_hexton=count_scmax_hexton+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,scmax_hexton
!!
      elseif(keyword.eq.'scale_min_hextoff')then
        if(count_scmin_hextoff.gt.0)then
          write(ounit,*)'Error: scale_min_hextoff specified twice'
          stop
        endif
        count_scmin_hextoff=count_scmin_hextoff+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,scmin_hextoff
!!
      elseif(keyword.eq.'scale_max_hextoff')then
        if(count_scmax_hextoff.gt.0)then
          write(ounit,*)'Error: scale_max_hextoff specified twice'
          stop
        endif
        count_scmax_hextoff=count_scmax_hextoff+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,scmax_hextoff
!!
      elseif(keyword.eq.'scale_min_dens')then
        if(count_scmin_dens.gt.0)then
          write(ounit,*)'Error: scale_min_dens specified twice'
          stop
        endif
        count_scmin_dens=count_scmin_dens+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,scmin_dens
!!
      elseif(keyword.eq.'scale_max_dens')then
        if(count_scmax_dens.gt.0)then
          write(ounit,*)'Error: scale_max_dens specified twice'
          stop
        endif
        count_scmax_dens=count_scmax_dens+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,scmax_dens
!!
...
!!
      elseif(keyword.eq.'use_old_weights_s')then
        if(count_luseoldweights_s.gt.0)then
          write(ounit,*)'Error: use_old_weights_s specified twice'
          stop
        endif
        count_luseoldweights_s=count_luseoldweights_s+1
        luseoldweights_s=.true.
!!
      elseif(keyword.eq.'use_old_weights_hexton')then
        if(count_luseoldweights_hexton.gt.0)then
          write(ounit,*)'Error: use_old_weights_hexton specified twice'
          stop
        endif
        count_luseoldweights_hexton=count_luseoldweights_hexton+1
        luseoldweights_hexton=.true.
!!
      elseif(keyword.eq.'use_old_weights_hextoff')then
        if(count_luseoldweights_hextoff.gt.0)then
          write(ounit,*)'Error: use_old_weights_hextoff specified twice'
          stop
        endif
        count_luseoldweights_hextoff=count_luseoldweights_hextoff+1
        luseoldweights_hextoff=.true.
!!
      elseif(keyword.eq.'use_old_weights_dens')then
        if(count_luseoldweights_dens.gt.0)then
          write(ounit,*)'Error: use_old_weights_dens specified twice'
          stop
        endif
        count_luseoldweights_dens=count_luseoldweights_dens+1
        luseoldweights_dens=.true.
...
!!
      elseif(keyword.eq.'write_trainhextoff')then
        if(count_lwritetrainhextoff.gt.0)then
          write(ounit,*)'Error: write_trainhextoff specified twice'
          stop
        endif
        count_lwritetrainhextoff=count_lwritetrainhextoff+1
        lwritetrainhextoff=.true.
!!
...
!!      don't do anything here, just let it pass
!!
      elseif(keyword.eq.'global_symfunction_hextoff')then
!!      don't do anything here, just let it pass
!!
...
!!
      elseif(keyword.eq.'shuffle_weights_short_atomic')then
        if(count_shuffle_weights_short_atomic.gt.0)then
          write(ounit,*)'Error: shuffle_weights_short_atomic specified twice'
          stop
        endif
        count_shuffle_weights_short_atomic=count_shuffle_weights_short_atomic+1
        backspace(nnunit)
        read(nnunit,*,ERR=99)dummy,nshuffle_weights_short_atomic,shuffle_weights_short_atomic
...
!!      don't do anything here, just let it pass
      elseif(keyword.eq.'global_activation_s')then
!!      don't do anything here, just let it pass
      elseif(keyword.eq.'global_activation_hexton')then
!!      don't do anything here, just let it pass
      elseif(keyword.eq.'global_activation_hextoff')then
!!      don't do anything here, just let it pass
      elseif(keyword.eq.'global_activation_dens')then
!!      don't do anything here, just let it pass
!!
...
!!
      elseif(keyword.eq.'nguyen_widrow_weights_ewald')then
        ...
        lnwweightse=.true.
!!
      elseif(keyword.eq.'print_date_and_time')then
        if(count_lprintdateandtime.gt.0)then
          write(ounit,*)'Error: print_date_and_time specified twice'
          stop
        endif
        count_lprintdateandtime=count_lprintdateandtime+1
        lprintdateandtime=.true.
!!
      elseif(keyword.eq.'enable_on_the_fly_input')then
        if(count_lenableontheflyinput.gt.0)then
          write(ounit,*)'Error: enable_on_the_fly_input specified twice'
          stop
        endif
        count_lenableontheflyinput=count_lenableontheflyinput+1
        lenableontheflyinput=.true.
!!
!! MG: Check for element-decoupled Kalman filter keyword
      elseif(keyword.eq.'element_decoupled_kalman')then
        if(count_luseedkalman.gt.0)then
          write(ounit,*)'Error: element_decoupled_kalman specified twice'
          stop
        endif
        count_luseedkalman=count_luseedkalman+1
        luseedkalman=.true.
!! MG: Keyword for 2nd Version of force fitting
      elseif(keyword.eq.'element_decoupled_forces_v2')then
        if(count_ledforcesv2.gt.0)then
          write(ounit,*)'Error: element_decoupled_forces_v2 specified twice'
          stop
        endif
        count_ledforcesv2=count_ledforcesv2+1
        ledforcesv2=.true.
!!
      elseif(keyword.eq.'analyze_composition')then
        if(count_lanalyzecomposition.gt.0)then
          write(ounit,*)'Error: analyze_composition specified twice'
          stop
        endif
        count_lanalyzecomposition=count_lanalyzecomposition+1
...
!!
      elseif(keyword.eq.'print_all_ham_weights')then
        write(pstring(5:5),'(a1)')'1'
!!
      elseif(keyword.eq.'print_all_s_weights')then
        write(pstring(6:6),'(a1)')'1'
!!
      elseif(keyword.eq.'print_all_hexton_weights')then
        write(pstring(7:7),'(a1)')'1'
!!
      elseif(keyword.eq.'print_all_hextoff_weights')then
        write(pstring(8:8),'(a1)')'1'
!!
      elseif(keyword.eq.'print_all_dens_weights')then
        write(pstring(9:9),'(a1)')'1'
