...
!!
      integer i1
!!
      if((mode.eq.2).and.(.not.(luseedkalman.and.luseforces)).and.(ledforcesv2))then
        write(ounit,*)'ERROR: element_decoupled_forces_v2 can only be used in combination'
        write(ounit,*)'with element_decoupled_kalman filter and force fitting'
        stop
      endif

!!SK:      if(luseipi)then
!!SK:        if(ipisocket.eq.0)then
!!SK:          write(ounit,*)'ERROR in specifying ipi socket in keyword use_ipi ',ipisocket
!!SK:          stop
!!SK:        endif
!!SK:      endif

      if((mode.eq.2).and.((luseforces).and.(luseedkalman)).and.(nforcegroup.gt.1))then
        write(ounit,*)'ERROR: element decoupled Kalman filter for forces'
        write(ounit,*)' cannot be used with force_grouping'
        stop
      endif

      if((mode.eq.2).and.((luseforces).and.(luseedkalman)).and.(lrepeate))then
        write(ounit,*)'ERROR: element decoupled Kalman filter for forces'
        write(ounit,*)' cannot be used with repeated energy update'
        stop
      endif
...
!!
      if((.not.lshort).and.(.not.lelec).and.(.not.lnntb))then
...
!!
      if((lnntb).and.(.not.nntb_flag(0)).and.(.not.nntb_flag(1))&
        .and.(.not.nntb_flag(2)).and.(.not.nntb_flag(3)).and.(.not.nntb_flag(4)))then
        write(ounit,*)'Error: NNTB mode not specified'
        stop
      endif
!!
      if((maxnum_layers_ham.eq.0).and.(lnntb).and.(nntb_flag(0)))then
        write(ounit,*)'Error: global_hidden_layers_ham is not specified'
        stop
      endif
!!
      if((maxnum_layers_s.eq.0).and.(lnntb).and.(nntb_flag(1)))then
        write(ounit,*)'Error: global_hidden_layers_s is not specified'
        stop
      endif
!!
      if((maxnum_layers_hexton.eq.0).and.(lnntb).and.(nntb_flag(2)))then
        write(ounit,*)'Error: global_hidden_layers_hexton is not specified'
        stop
      endif
!!
      if((maxnum_layers_hextoff.eq.0).and.(lnntb).and.(nntb_flag(3)))then
        write(ounit,*)'Error: global_hidden_layers_hextoff is not specified'
        stop
      endif
!!
      if((maxnum_layers_dens.eq.0).and.(lnntb).and.(nntb_flag(4)))then
        write(ounit,*)'Error: global_hidden_layers_dens is not specified'
        stop
      endif
...
!!
      if((count_nodes_ham.eq.0).and.(lnntb).and.(nntb_flag(0)))then
        write(ounit,*)'Error: global_nodes_ham is not specified'
        stop
      endif
!!
      if((count_nodes_s.eq.0).and.(lnntb).and.(nntb_flag(1)))then
        write(ounit,*)'Error: global_nodes_s is not specified'
        stop
      endif
!!
      if((count_nodes_hexton.eq.0).and.(lnntb).and.(nntb_flag(2)))then
        write(ounit,*)'Error: global_nodes_hexton is not specified'
        stop
      endif
!!
      if((count_nodes_hextoff.eq.0).and.(lnntb).and.(nntb_flag(3)))then
        write(ounit,*)'Error: global_nodes_hextoff is not specified'
        stop
      endif
!!
      if((count_nodes_dens.eq.0).and.(lnntb).and.(nntb_flag(4)))then
        write(ounit,*)'Error: global_nodes_dens is not specified'
        stop
      endif
...
!!
      do i1=1,npairs
        if(lnntb.and.(nntb_flag(0)))then
          if(nodes_ham(0,i1).ne.num_funcvalues_ham(i1))then
            write(ounit,*)'Error: num_funcvalues_ham .ne. nodes_ham(0)',&
              num_funcvalues_ham(i1),nodes_ham(0,i1)
            write(ounit,*)'Did you set the right number of input nodes?'
            stop
          endif
        endif
...
!!
      do i1=1,npairs
        if(lnntb.and.(nntb_flag(1)))then
          if(nodes_s(0,i1).ne.num_funcvalues_s(i1))then
            write(ounit,*)'Error: num_funcvalues_s .ne. nodes_s(0)',&
              num_funcvalues_s(i1),nodes_s(0,i1)
            write(ounit,*)'Did you set the right number of input nodes?'
            stop
          endif
        endif
...
!!
      do i1=1,npairs
        if(lnntb.and.(nntb_flag(2)))then
          if(nodes_hexton(0,i1).ne.num_funcvalues_hexton(i1))then
            write(ounit,*)'Error: num_funcvalues_hexton .ne. nodes_hexton(0)',&
              num_funcvalues_hexton(i1),nodes_hexton(0,i1)
            write(ounit,*)'Did you set the right number of input nodes?'
            stop
          endif
        endif
      ...
      if(mode.eq.3)then
        do i1=1,ntriplets
          if(lnntb.and.nntb_flag(3))then
            if(nodes_hextoff(0,i1).ne.num_funcvalues_hextoff(i1))then
              write(ounit,*)'Error: num_funcvalues_hextoff .ne. nodes_hextoff(0)',&
                num_funcvalues_hextoff(i1),nodes_hextoff(0,i1)
              write(ounit,*)'Did you set the right number of input nodes?'
              stop
            endif
          endif
...
!!
      do i1=1,npairs
        if(lnntb.and.(nntb_flag(4)))then
          if(nodes_dens(0,i1).ne.num_funcvalues_dens(i1))then
            write(ounit,*)'Error: num_funcvalues_dens .ne. nodes_dens(0)',&
              num_funcvalues_dens(i1),nodes_dens(0,i1)
            write(ounit,*)'Did you set the right number of input nodes?'
            stop
          endif
        endif
...
!!
      if(lnntb.and.nntb_flag(0))then
        if(count_global_activation_ham.eq.0)then
...
!!
      if(lnntb.and.(nntb_flag(1)))then
        if((count_global_activation_s.eq.0))then
          write(ounit,*)'Error: global_activation_s is not specified'
          stop
        endif
      endif
!!
      if(lnntb.and.(nntb_flag(2)))then
        if((count_global_activation_hexton.eq.0))then
          write(ounit,*)'Error: global_activation_hexton is not specified'
          stop
        endif
      endif
!!
      if(lnntb.and.(nntb_flag(3)).and.(mode.ge.2))then
        if((count_global_activation_hextoff.eq.0))then
          write(ounit,*)'Error: global_activation_hextoff is not specified'
          stop
        endif
      endif
!!
      if(lnntb.and.(nntb_flag(4)))then
        if((count_global_activation_dens.eq.0))then
          write(ounit,*)'Error: global_activation_dens is not specified'
          stop
        endif
      endif
!!
...
!!
      if(lcentersym.and.(count_scmin_s.gt.0))then
        write(ounit,'(a)')'Error: center_symmetry_functions cannot be combined with scale_min_s keyword'
        stop
      endif
!!
      if(lcentersym.and.(count_scmax_s.gt.0))then
        write(ounit,'(a)')'Error: center_symmetry_functions cannot be combined with scale_max_s keyword'
        stop
      endif
!!
      if(lcentersym.and.(count_scmin_hexton.gt.0))then
        write(ounit,'(a)')'Error: center_symmetry_functions cannot be combined with scale_min_hexton keyword'
        stop
      endif
!!
      if(lcentersym.and.(count_scmax_hexton.gt.0))then
        write(ounit,'(a)')'Error: center_symmetry_functions cannot be combined with scale_max_hexton keyword'
        stop
      endif
!!
      if(lcentersym.and.(count_scmin_hextoff.gt.0))then
        write(ounit,'(a)')'Error: center_symmetry_functions cannot be combined with scale_min_hextoff keyword'
        stop
      endif
!!
      if(lcentersym.and.(count_scmax_hextoff.gt.0))then
        write(ounit,'(a)')'Error: center_symmetry_functions cannot be combined with scale_max_hextoff keyword'
        stop
      endif
!!
      if(lcentersym.and.(count_scmin_dens.gt.0))then
        write(ounit,'(a)')'Error: center_symmetry_functions cannot be combined with scale_min_dens keyword'
        stop
      endif
!!
      if(lcentersym.and.(count_scmax_dens.gt.0))then
        write(ounit,'(a)')'Error: center_symmetry_functions cannot be combined with scale_max_dens keyword'
        stop
      endif
...
!!
      if((count_scmin_s.gt.0).and.(.not.lscalesym))then
        write(ounit,*)'Error: scale_min_s requires keyword scale_symmetry_functions'
        stop
      endif
!!
      if((count_scmax_s.gt.0).and.(.not.lscalesym))then
        write(ounit,*)'Error: scale_max_s requires keyword scale_symmetry_functions'
        stop
      endif
!!
      if((count_scmin_hexton.gt.0).and.(.not.lscalesym))then
        write(ounit,*)'Error: scale_min_hexton requires keyword scale_symmetry_functions'
        stop
      endif
!!
      if((count_scmax_hexton.gt.0).and.(.not.lscalesym))then
        write(ounit,*)'Error: scale_max_hexton requires keyword scale_symmetry_functions'
        stop
      endif
!!
      if((count_scmin_hextoff.gt.0).and.(.not.lscalesym))then
        write(ounit,*)'Error: scale_min_hextoff requires keyword scale_symmetry_functions'
        stop
      endif
!!
      if((count_scmax_hextoff.gt.0).and.(.not.lscalesym))then
        write(ounit,*)'Error: scale_max_hextoff requires keyword scale_symmetry_functions'
        stop
      endif
!!
      if((count_scmin_dens.gt.0).and.(.not.lscalesym))then
        write(ounit,*)'Error: scale_min_dens requires keyword scale_symmetry_functions'
        stop
      endif
!!
      if((count_scmax_dens.gt.0).and.(.not.lscalesym))then
        write(ounit,*)'Error: scale_max_dens requires keyword scale_symmetry_functions'
        stop
      endif
...
!!
      if(scmin_s.ge.scmax_s)then
        write(ounit,'(a)')'Error: scale_min_s .ge. scale_max_s'
        stop
      endif
!!
      if(scmin_hexton.ge.scmax_hexton)then
        write(ounit,'(a)')'Error: scale_min_hexton .ge. scale_max_hexton'
        stop
      endif
!!
      if(scmin_hextoff.ge.scmax_hextoff)then
        write(ounit,'(a)')'Error: scale_min_hextoff .ge. scale_max_hextoff'
        stop
      endif
!!
      if(scmin_dens.ge.scmax_dens)then
        write(ounit,'(a)')'Error: scale_min_dens .ge. scale_max_dens'
        stop
      endif
