...
!! - readinput.f90
!!
      subroutine sorthextoffsymfunctions(ndim,ntriplets,tripletid,mode,&
...
!!
      integer i1,i2
      integer ndim                                    ! in
      integer ntriplets                               ! in
      integer tripletid                               ! in
      integer mode                                    ! in
      integer maxnum_funcvaluesp                      ! in
      integer num_funcvaluesp(ndim)                   ! in
      integer function_type_temp                      ! internal
      integer function_typep(maxnum_funcvaluesp,ndim) ! in/out
      integer sympelement(maxnum_funcvaluesp,2,ndim)  ! in/out
      integer sympelement_temp(2)                     ! internal
      integer counter(6,ndim)                         ! internal
      integer elemtriplet(ntriplets,3)
!!
      real*8 funccutoffp(maxnum_funcvaluesp,ndim)     ! in/out
      real*8 etap(maxnum_funcvaluesp,ndim)               ! in/out
      real*8 zetap(maxnum_funcvaluesp,ndim)              ! in/out
      real*8 lambdap(maxnum_funcvaluesp,ndim)            ! in/out
      real*8 rshiftp(maxnum_funcvaluesp,ndim)            ! in/out
      ...
      do i1=1,ndim
        do i2=1,num_funcvaluesp(i1)-1
          if((function_typep(i2,i1).eq.function_typep(i2+1,i1))&
            ...
            if(mode.eq.3)then
              write(ounit,*)'triplet is ',elemtriplet(i1,1),elemtriplet(i1,2),elemtriplet(i1,3)
            else
              write(ounit,*)'triplet is ',elemtriplet(tripletid,1),&
                elemtriplet(tripletid,2),elemtriplet(tripletid,3)
            endif
