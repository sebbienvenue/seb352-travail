...
!!
      subroutine ewaldreal(max_num_neighbors_elec,num_neighbors_elec,&
...
!!
          ereal=ereal+atomcharge(i)*atomcharge(n)*&
...
!!
            do i1=0,num_neighbors_elec(i)
              if(neighboridx_elec(i,i1).eq.i) then
                drijd(1)=drijdxi
                drijd(2)=drijdyi
                drijd(3)=drijdzi
              elseif(neighboridx_elec(i,i1).eq.n) then
                drijd(1)=drijdxj
                drijd(2)=drijdyj
                drijd(3)=drijdzj
              else
...
!!
            ...
            erealforce(i2,neighboridx_elec(i,i1))=erealforce(i2,neighboridx_elec(i,i1))&
              -((atomcharge(n)*dchargedxyz(i,i1,i2)  &
                +atomcharge(i)*dchargedxyz(n,i1,i2)) &
              *comperrfct(temp1)/rij                 &
              +atomcharge(i)*atomcharge(n)*          &
