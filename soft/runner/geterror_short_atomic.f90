...
!! called by:
!!
      subroutine geterror_short_atomic(iswitch,countepoch,ntrain,&
        imaxerror_eshort,&
      ...
      if(mpirank.eq.0)then
        if(iswitch.eq.0)then
          ...
          if(lwritetrainpoints)then
            ...
            rewind(trainpxunit) !'
            write(trainpxunit,'(a)')' point      Etot(DFT)      Etot(NN)  '
...
!!
        elseif(iswitch.eq.1)then ! test error
          ...
          if(lwritetrainpoints)then
            ...
            rewind(testpxunit) !'
            write(testpxunit,'(a)')' point      Etot(DFT)      Etot(NN)  '
