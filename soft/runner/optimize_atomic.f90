...
!! Purpose: update the weights for the short range atomic case using
!!          energies, forces and (in case of nn_type_elec 2) charges

!! called by:
!!
!! This routine is called once for each block of points
!!
      subroutine optimize_atomic(npoints,point,idx,&
         ...
         rmse_short,rmse_force_s_ref,rmse_charge,&
...
!!
      ...
      integer fitstate(ntrain)                                          ! in/out
...
!!
      ...
      real*8 errorq                                                     ! internal
      real*8 abserrorq                                                  ! internal
      ...
      real*8 deshortdw(maxnum_weights_short_atomic,nelem)               ! internal
      real*8 deshortdwsum(maxnum_weights_short_atomic,nelem)            ! internal
      real*8 dchargedw(maxnum_weights_short_atomic,max_num_atoms)       ! internal
      ...
      real*8 scalefactorqtemp(nelem)                                    ! internal
      real*8 energyerror_list(nblock)                                   ! internal
      real*8 sumwsquared                                                ! internal
      real*8 errorthresholde                                            ! internal 
      real*8 errorthresholdf                                            ! internal 
      real*8 errorthresholdq                                            ! internal 
...
!!
!!======================================================
!! determine mchargegroup for each element and make sure it is not larger than the number of charges for this element in this block of structures
!!======================================================
...
!!
!!======================================================
!! initialize total number of free short range atomic weights
!!======================================================
...
!!
      if(luseworstq)then
        write(ounit,*)'ERROR: use worst charges is not supported for nn_type_elec 2'
        stop !'
      endif
...
!!
          if((mpirank.eq.0).and.ldebug) then
            write(debugunit,'(a15,i8,a11,i8)')&
              'Updating point ',point,' structure ',idx(i1)+point-i1
          endif
!!
!!======================================================
...
!!
!!======================================================
!! determine which natoms atoms of this structure should be calculated by this process
!!======================================================
          call mpifitdistribution(num_atoms,natoms,n_start,n_end)
!!
!!======================================================
!! determine the atomindex array for the identification of atoms
!!======================================================
...
!!======================================================
          ...
          nnatomcharge(:)= 0.0d0
          nntotalcharge  = 0.0d0
          call getatom_para(natoms,atomindex,&
            zelem,symfunction_short_atomic_list(1,n_start,idx(i1)),&
            nnatomenergy,nnatomcharge)
!!
!!======================================================
!! combine nnatomenergy array from all processes and calculate total energy 
!!======================================================
...
!!======================================================
!! combine nnatomcharge array from all processes and calculate total charge 
!!======================================================
          if(lelec.and.(nn_type_elec.eq.2))then
            call mpi_allreduce(mpi_in_place,nnatomcharge,&
              max_num_atoms,mpi_real8,mpi_sum,mpi_comm_world,mpierror)
            nntotalcharge=0.0d0
            do i2=1,num_atoms
              nntotalcharge=nntotalcharge+nnatomcharge(i2)
            enddo
          endif
!!
!!======================================================
!! calculate the energy error (per atom) of this training point
!!======================================================
...
!!
!!======================================================
!! set errorthresholde criterion for short range energy
!!======================================================
          if(lfixederrore)then
            errorthresholde=fixederrore
          else
            errorthresholde=kalmanthreshold_temp*rmse_short
          endif
!!
!!======================================================
!!======================================================
...
!!======================================================
!!======================================================
          if((abserrore.gt.errorthresholde)&
            .or.&
            (i1.eq.npoints))then ! last point of this block reached, maybe update needs to be done
!!
            if(abserrore.le.errorthresholde) goto 77 ! don't use this gradient because point is good enough
!!
!! do some counting
            fitstate(ndone+idx(i1))=fitstate(ndone+idx(i1))+1
            nenergy=nenergy+1
            numbere=numbere+1
!!
!! calculate the derivative deshortdw of the short range energy with respect to the weights
            ...
            call getdnnoutdw(num_weights_short_atomic_free,&
              zelem,num_atoms,wconstraintidx,&
              symfunction_short_atomic_list(1,1,idx(i1)),deshortdw,dchargedw)
              ...
              do i3=1,nelem
                do i4=1,maxnum_weights_short_atomic
                  write(debugunit,'(2i6,a,2i6,f20.10)')&
                    countepoch,point,&
                    ' deshortdw ',i3,i4,deshortdw(i4,i3)
                enddo
              enddo
            endif
!!
            ...
            if(ldebug)then
              ...
              do i3=1,nelem !'
                ...
                do i4=1,num_weights_short_atomic(i3)
                  debugsum=debugsum+deshortdw(i4,i3)**2
                  debugsum2=debugsum2+deshortdw(i4,i3)
                enddo ! i4
            ...
            if(ldampw) then
              do i2=1,nelem
                do i3=1,num_weights_short_atomic_free(i2)
                  deshortdw(i3,i2)=(1.d0-dampw)*deshortdw(i3,i2) &
...
!! reduce full weight array and deshortdw to arrays containing only the free weights
                    do i3=1,num_weights_short_atomic_free(i2)
                      ...
                      deshortdw_temp(i3)=deshortdw(i3,i2)
                    enddo ! i3
!!
...
!!======================================================
!!======================================================
        call abstime(timeqfittingstart,dayqfitting)
!!
        if(lelec.and.(nn_type_elec.eq.2).and.(chargernd.gt.0.0d0))then
!!
          do i2=1,nelem
            if(scalefactorq.lt.0.0d0)then
              scalefactorqtemp(i2)=(dble(mchargegroup(i2))*chargernd)/&
                        (dble(num_atoms_element_struct(i2)*dble(nchargegroup))*chargernd)
            else ! keep scalefactorq from input.nn
              scalefactorqtemp(i2)=scalefactorq
            endif
          enddo
!!
...
!!
        call abstime(timeqfittingend,dayqfitting)
        timeqfitting=timeefitting+(timeqfittingend-timeqfittingstart)
!!
!!======================================================
!!======================================================
!! force part
...
!!
!! set errorthresholdf criterion for forces 
...
!! adapt sign 
                        ...
                        do i5=1,num_weights_short_atomic_free(i4)
                          deshortdw_temp(i5)=deshortdw(i5,i4)
                        enddo ! i5
...
!! get joint derivative 
          ...
          do i2=1,nelem
            deshortdw(:,i2)=(deshortdw(:,i2)+dfshortdwsum(:,i2))/2.d0
          enddo ! i2
!!
          do i2=1,nelem
            allocate(weights(num_weights_short_atomic_free(i2)))
