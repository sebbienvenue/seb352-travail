...
!!
        ...
        subroutine getoneshortforcepair_para(iatom,ixyz,&
                   pindex,num_pairs,zelemp, &
                   symfunctionp,dsfuncdxyz_pair,nnshortforce)
!!
      use fileunits
      use globaloptions
      use nnshort_pair
...
!!
        symfunction_pair(:)=symfunctionp(:,pindex(i1))
        weightsp(:)=weights_short_pair(:,pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1))))
...
!! in order to save memory
        call calconenn(1,maxnum_funcvalues_short_pair,maxnodes_short_pair,&
          ...
          num_layers_short_pair(pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1)))),&
          maxnum_weights_short_pair,&
          nodes_short_pair(0,pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1)))),symfunction_pair,&
...
!! calculate part of the derivative at each node (\frac{\partial f_a(G_i)}{\partial G_i} )
        ipairs=pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1)))
!!
...
!! => nodes_short_atomic(1) values for each input node
        do i2=1,nodes_short_pair(1,pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1))))              ! over all nodes in layer 1 ("target layer") 
          do i3=1,num_funcvalues_short_pair(pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1))))         ! over all nodes in previous layer
            icount=(i3-1)*nodes_short_pair(1,pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1))))+i2 ! set pointer in weights array, don't need windex_short_atomic for first weights
            tempderivativep(1,i2,i3)=dnodes_values(1,i2)*weightsp(icount)
          enddo ! i3
        enddo ! i2

!! for layers 2 and beyond (if present) 
        if(num_layers_short_pair(pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1)))).gt.1)then
          do i2=2,num_layers_short_pair(pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1))))  ! over all hidden and output layers
            do i3=1,nodes_short_pair(i2,pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1)))) ! over all nodes in the target layer

              do i5=1, num_funcvalues_short_pair(pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1))))
!! we have to sum over the nodes in the previous layer (i4)
                do i4=1,nodes_short_pair(i2-1,pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1)))) ! sum over all nodes in previous layer
                  icount=windex_short_pair(2*i2-1, pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1))))&
                 +(i4-1)*nodes_short_pair(i2,pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1))))+i3-1 
...
!!
        depairdsfunc(pindex(i1),:)=tempderivativep(num_layers_short_pair(pairindex(zelemp(1,pindex(i1)),zelemp(2,pindex(i1)))),1,:)
!!
! 98     continue
      enddo ! i1---> num_pairs 
...
!! don't calculate forces that we don't need
!      if(lupdatebyelement.and.(zelem(i1).ne.elemupdate)) goto 99
...
!          do i3=1,3 ! x,y,z
            do i4=1,num_funcvalues_short_pair(pairindex(zelemp(1,i2),zelemp(2,i2)))  ! over all symmetry functions
              ...
              -depairdsfunc(i2,i4)*dsfuncdxyz_pair(i4,i2,iatom,ixyz)
            enddo ! i4
!          enddo ! i3
        enddo ! i2
!! 99     continue
